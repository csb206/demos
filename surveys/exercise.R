...
probWeights <- table(apipop$stype)/table(apistrat$stype)
...
nofpcDesign <- svydesign(id=~1,strata=~stype, weights=~pw, data=apistrat)
...
dclus<-svydesign(id=~dnum, weights=~pw, fpc=~fpc, data=apiclus1)
